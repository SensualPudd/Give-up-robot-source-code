   ...
   public class Entity extends Core
   {
      ...
      public function getNearestType(type:String, x:Number, y:Number, useHitBox:Boolean = false) : Entity
      {
         ...
         if(useHitBox)
         {
            ...
            while(e)
            {
               ...
               {
                  dist = FP.distanceRects(x,y,this.width,this.height,e.x - e.originX,e.y - e.originY,e.width,e.height);
                  if(dist < nearDist)
                  {
                     nearDist = dist;
                     near = e;
         ...
         if(this._added && value != this._depth)
         {
            if(value > this._depth)
            {
               if(this._renderNext && this._renderNext._depth < value)
               {
                  ...
                  {
                     FP.world._renderFirst = this._renderNext;
                  }
      ...
      public function collidePoint(x:int, y:int) : Boolean
      {
         if(x >= this.x - this.originX && y >= this.y - this.originY && x < this.x - this.originX + this.width && y < this.y - this.originY + this.height)
         {
            ...
            return this.checkBack(FP.entity,x,y);
         }
         return false;
      }
      ...
      public function collideRect(rx:int, ry:int, rwidth:int, rheight:int) : Boolean
      {
         if(this.x - this.originX + this.width > rx && this.y - this.originY + this.height > ry && this.x - this.originX < rx + rwidth && this.y - this.originY < ry + rheight)
         {
            if(this.collideBack)
            {
               ...
               return this.checkBack(FP.entity,rx,ry);
            }
            if(!this.mask)
            {
               return true;
      ...
      public function distanceTo(e:Entity, x:int, y:int) : Number
      {
         x = x - this.originX;
         y = y - this.originY;
         return FP.distanceRects(x,y,this.width,this.height,e.x - e.originX,e.y - e.originY,e.width,e.height);
      }
      ...
      public function set type(value:String) : void
      {
         ...
         if(this._collisionType && FP.world._collisionFirst[this._collisionType])
         {
            if(FP.world._collisionFirst[this._collisionType] == this)
            {
               FP.world._collisionFirst[this._collisionType] = this._collisionNext;
            }
         ...
         {
            this._collisionNext = FP.world._collisionFirst[value];

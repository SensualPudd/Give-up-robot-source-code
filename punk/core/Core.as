package punk.core
{
   ...
   public class Core
   {
      ...
      public function drawRect(x:int, y:int, w:int, h:int, color:uint = 0, alpha:Number = 1) : void
      {
         if(alpha >= 1)
         {
            this._rect.x = x - FP.camera.x;
            this._rect.y = y - FP.camera.y;
            this._rect.width = w;
            this._rect.height = h;
            FP.screen.fillRect(this._rect,color);
            return;
         }
         this._graphics.clear();
         this._graphics.beginFill(color,alpha);
         this._graphics.drawRect(x - FP.camera.x,y - FP.camera.y,w,h);
         this._graphics.endFill();
         FP.screen.draw(this._sprite);
      }
      ...
      public function drawLinePlus(x1:int, y1:int, x2:int, y2:int, color:uint = 4.27819008E9, alpha:Number = 1, thick:Number = 1) : void
      {
         ...
         this._graphics.moveTo(x1 - FP.camera.x,y1 - FP.camera.y);
         this._graphics.lineTo(x2 - FP.camera.x,y2 - FP.camera.y);
         FP.screen.draw(this._sprite);
      }
      ...
      public function drawSprite(sprite:Spritemap, image:int = 0, x:int = 0, y:int = 0, flipX:Boolean = false, flipY:Boolean = false) : void
      {
         ...
         this._point.x = x - FP.camera.x - sprite.originX;
         this._point.y = y - FP.camera.y - sprite.originY;
         FP.screen.copyPixels(sprite,this._rect,this._point);
      }
      ...
      public function drawCircle(x:int, y:int, radius:Number, color:uint = 0, alpha:Number = 1) : void
      {
         this._graphics.clear();
         this._graphics.beginFill(color,alpha);
         this._graphics.drawCircle(x - FP.camera.x,y - FP.camera.y,radius);
         this._graphics.endFill();
         FP.screen.draw(this._sprite);
      }
   }

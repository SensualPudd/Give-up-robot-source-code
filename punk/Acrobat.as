   ...
   public class Acrobat extends Actor
   {
      ...
      override public function render() : void
      {
         ...
         if(this.angle == 0 && this.scaleX == 1 && this.scaleY == 1)
         {
            ...
            FP.screen.copyPixels(this._buffer,this._bufferRect,_point);
            if(!delay)
            {
               return;
            }
         ...
         if(this.center)
         {
            ...
            _matrix.tx = _matrix.tx + (x - FP.camera.x - _sprite.originX + _sprite.imageCX);
            _matrix.ty = _matrix.ty + (y - FP.camera.y - _sprite.originY + _sprite.imageCY);
         }
         else
         {
            ...
            _matrix.tx = _matrix.tx + (x - FP.camera.x);
            _matrix.ty = _matrix.ty + (y - FP.camera.y);
         }
         FP.screen.draw(this._buffer,_matrix);
         if(!delay)
         {
            return;
         }
